...
[![源码版本: v1.0.13](https://img.shields.io/badge/source-v1.0.13-blue)](package.json)
...
支持 macOS、Windows 和 Linux，使用 Rust、Tauri 2 和 React 编写。果到雷达的正式版本从 **1.0.0** 开始，本 README 对应 `1.0.13` 源码。应用名称为 **果到雷达**，仓库名为 `apple-store-inventory-monitor`。
...
`v1.0.2` 的 macOS 包曾因应用签名不完整而被系统误报为“已损坏”，此问题已在 `v1.0.3` 修复。如果新版仍显示旧提示，请先删除旧应用和旧 DMG，确认重新下载的文件名包含 `1.0.13`。确认安装包来自本仓库 Release 后，仍可执行：
...
```bash
sudo apt install "./apple-store-inventory-monitor_1.0.13_amd64.deb"
```
...
```bash
chmod +x "./apple-store-inventory-monitor_1.0.13_amd64.AppImage"
"./apple-store-inventory-monitor_1.0.13_amd64.AppImage"
```

AppImage 需要 FUSE 2。Ubuntu 24.04 可以用 `sudo apt install libfuse2t64` 安装；也可以不安装 FUSE，直接解包运行：

```bash
"./apple-store-inventory-monitor_1.0.13_amd64.AppImage" --appimage-extract-and-run
```
...
正式版本由 [Release 工作流](.github/workflows/release.yml) 构建，标签使用 `v1.0.13` 这样的格式。发版前应确认 `package.json`、工作区 `Cargo.toml`、`src-tauri/tauri.conf.json` 与标签的版本一致。macOS 专用配置在 `src-tauri/tauri.macos.conf.json` 中启用 ad-hoc 签名；发布流水线会同时验证自动更新归档和 DMG 内的应用签名。
