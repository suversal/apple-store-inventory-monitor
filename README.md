...
果到雷达是一款 Apple 直营店取货库存监控工具。选好地区、门店和具体型号后，它会定时检查库存；检测到有货时，可以播放提示音、推送 Bark，并打开监控商品的 Apple 购买页面。
...
截图中的 Bark 密钥已遮挡；库存仅代表拍摄时结果。截图来自更新前的本地版本，日志中的旧提醒记录不代表当前版本提供系统通知。当前提醒方式为提示音、Bark 和自动打开商品页。
...
- 支持提示音和 Bark 推送。
- 有货时可以自动打开对应地区、对应型号的 Apple 商品页；也可以点击监控列表中的型号手动打开。
- 窗口关闭后可继续在系统托盘运行。
...
如果启用了「自动打开商品页」，持续有货也会每轮再次打开页面。只想听到提示音或收到 Bark、不想反复出现浏览器标签页时，请关闭这个选项。
...
- 自动打开商品页：检测到有货后调用系统默认浏览器，打开对应地区的商品配置页。iPhone 等机型按零件号打开；Apple Watch 表壳需要搭配表带，进入对应系列配置页继续选择。不会自动添加监控商品、预留库存或下单。
- Bark 提醒中的链接与商品页跳转一致。已有配置沿用 `openBagOnHit` 字段，无需重新设置。

「测试提醒与跳转」按当前设置测试提示音、Bark 和商品页跳转，不查询库存，也不代表有货。开启自动跳转且已有监控目标时，会打开第一个目标的商品页；只测试跳转时须先添加目标。实际是否收到提醒，需要在电脑或手机上确认。
...
| 操作反馈 | 有货时会执行提醒并尝试打开购物袋，但界面不显示具体执行结果 | 记录已执行的提醒动作，并显示提示音、Bark 或商品页打开失败的错误；实际送达情况仍需在设备上确认 |
