...
| 应用与配置 | 使用 `apple-pickup-watcher` 名称、应用标识和配置目录 | 更名为 Apple Store Inventory Monitor（果到雷达），使用独立应用标识和配置目录；首次启动时只读迁移旧设置，旧文件不会被删除 |
| 发布与更新 | 使用上游仓库和更新身份 | 提供本项目自己的 macOS Apple 芯片、macOS Intel、Windows x64 和 Linux x64 安装包；自动更新包使用独立的 Tauri 更新密钥签名 |
